...
def smart_match(description, activity_set, activity_dict , top_n=1):
    ...
    if partial_matches:
        ...
        top_matches = partial_matches[:top_n]
        codes = [str(match[1]).zfill(6) for match in top_matches]
        suggestions = [f"{match[0]} (تشابه: {match[2]}%)" for match in top_matches]
